EVALUATION LICENSE AGREEMENT

Version 2.1 — © Thomas Price. All Rights Reserved.

This Evaluation License Agreement (“Agreement”) governs all access to and use of the software, source code, binaries, documentation, designs, architectures, algorithms, cognitive systems, memory structures, compression engines, and related materials (“Software”) provided by Thomas Price (“Licensor”).



By accessing, downloading, installing, compiling, executing, or otherwise using the Software, the receiving party (“Evaluator”) agrees to all terms below.



1\. GRANT OF LICENSE (EVALUATION ONLY)

Licensor grants Evaluator a limited, non‑exclusive, non‑transferable, non‑assignable, non‑sublicensable, revocable license to use the Software solely for internal, non‑commercial evaluation (“Evaluation Purpose”).



No other rights are granted.

No rights are implied.



2\. PROHIBITED ACTIONS (ABSOLUTE RESTRICTIONS)

Evaluator SHALL NOT, under any circumstances:



2.1 Copying \& Reproduction

Copy, reproduce, mirror, or duplicate the Software, in whole or in part.



2.2 Modification \& Derivative Works

Modify, alter, translate, adapt, patch, extend, or create derivative works from the Software.



Evaluator may NOT re‑implement, emulate, approximate, or derive any part of the Software’s:



architecture



memory system



drift physics



reinforcement logic



semantic branching



cognitive behavior



dynamic webbing



pruning system



clustering system



graph engine



compression engine



BD3D collapse logic



Origami compression mode



PyMid structured‑text encoder/decoder



NUMBIN counter compressor



adaptive compression profile router



GPU BD3D encoder/decoder



compressed cognitive‑snapshot formats



compressed drift packets



compressed branch metadata formats



compressed heatmap formats



compressed BFS / PageRank formats



compressed KV‑subset routing formats



2.3 Reverse Engineering

Evaluator may NOT reverse‑engineer, decompile, disassemble, analyze, inspect, or attempt to derive:



source code



internal structures



algorithms



cognitive patterns



architectural concepts



semantic or procedural memory behavior



compression rules



collapse physics



PTS‑style reversible mapping



adaptive profile heuristics



GPU compression behavior



compressed memory formats



2.4 Unauthorized Use

Evaluator may NOT use the Software:



in production



in commercial products



in internal company systems



in research



in academic work



in robotics



in autonomous systems



in AI agents



in machine learning pipelines



in any paid or unpaid project



in any system that interacts with customers, users, or hardware



2.5 Competitive Restrictions

Evaluator may NOT:



build competing products



build similar products



build derivative products



build products inspired by the Software



build products using the same architecture



build products using the same memory system



build products using the same drift physics



build products using the same semantic branching



build products using the same cognitive patterns



build products using the same compression engine



build products using BD3D, Origami, PyMid, NUMBIN, or adaptive compression concepts



build products using reversible cognitive‑snapshot compression



2.6 AI/ML Restrictions

Evaluator may NOT:



train AI models using the Software



use the Software as training data



use the Software as architectural inspiration



use the Software to design competing cognitive systems



use the Software to evaluate or benchmark AI systems



use the Software’s compression engine to train or optimize AI models



use compressed outputs to infer internal cognitive behavior



2.7 Distribution

Evaluator may NOT:



distribute



publish



sell



lease



rent



loan



upload



or otherwise make the Software available to any third party



2.8 Intellectual Extraction

Evaluator may NOT use any ideas, structures, patterns, designs, or concepts contained in the Software outside of the Evaluation Purpose.



This includes:



cognitive architecture



semantic memory design



drift physics



pruning physics



dynamic webbing



branch‑aware semantics



compression physics



collapse rules



adaptive profile routing



GPU compression behavior



compressed memory formats



Any prohibited action immediately terminates this Agreement.



3\. OWNERSHIP

Licensor retains all right, title, and interest in and to the Software, including all intellectual property rights.



Evaluator acknowledges that:



The Software is proprietary.



The Software contains trade secrets.



The Software’s architecture, design, cognitive behavior, memory systems, and compression systems are protected intellectual property.



No ownership or rights are transferred.



4\. CONFIDENTIALITY

Evaluator agrees to treat the Software as confidential and proprietary information.

Evaluator shall not disclose the Software or any information derived from it to any third party without Licensor’s written consent.



5\. NO WARRANTY

The Software is provided “AS IS” without warranty of any kind, express or implied.



6\. LIMITATION OF LIABILITY

Licensor shall not be liable for any damages arising from Evaluator’s use or inability to use the Software.



7\. TERMINATION

This Agreement automatically terminates if Evaluator violates any term herein.



Upon termination, Evaluator must:



Immediately cease all use of the Software.



Permanently delete all copies of the Software.



Certify deletion in writing upon request.



8\. COMMERCIAL LICENSE REQUIRED

Evaluator acknowledges and agrees that ANY use of the Software beyond the Evaluation Purpose — including but not limited to:



production use



derivative works



architectural reuse



competitive development



integration



cognitive system development



AI agent integration



memory system replication



compression engine reuse



collapse rule reuse



adaptive profile reuse



GPU compression reuse



compressed memory format reuse



REQUIRES a paid commercial license negotiated directly with Licensor.



Evaluator’s ONLY path to continued use is purchasing a commercial license.



9\. GOVERNING LAW

This Agreement shall be governed by the laws of the United States and the State of Kentucky, without regard to conflict‑of‑law principles.



10\. ENTIRE AGREEMENT

This Agreement constitutes the entire agreement between the parties regarding the Evaluation Purpose.



© Thomas Price. All Rights Reserved.

